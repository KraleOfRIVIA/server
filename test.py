...
# Наложение прямоугольных рамок и текста на изображение
for box in data["boxes"]:
    x, y, w, h, confidence, object_key = box
    x, y, w, h = int(x), int(y), int(w), int(h)
    object_name = object_names.get(str(int(object_key)), "Unknown")
    cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)
    cv2.putText(img, f"{object_name}: {confidence:.2f}", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

# Показываем изображение с наложенными прямоугольными рамками и текстом
